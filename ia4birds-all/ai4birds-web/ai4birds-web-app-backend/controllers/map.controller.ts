...
const getExclusionMapDataStreaming = async (req: Request, res: Response) => {
    ...
    axios
        .post(
            `${globalConfig.pythonURL}/exclusionmap/stream-exclusion-data?id=${newClient.id}`,
        )
        .then((response) => {
            if (response.status !== 200) {
                throw new Error(
                    "No se pudieron obtener los datos del mapa de exclusión eólica en Stream.",
                );
            } else {
...
function sendEventToClient(newFact, clientId) {
    ...
    if (client && client.res) {
        ...
        console.error(
            `Client with ID ${clientId} not found or client.res is undefined`,
        );
    }
}
...
async function processZip(response: any): Promise<any[]> {
    ...
    const jsonDataEntry = zipEntries.find(
        (entry) => entry.entryName === "data.json",
    );
    ...
    if (!existsSync(tempFilePath)) {
        throw new Error(
            `El archivo ${tempFilePath} no se encuentra o no se extrajo correctamente.`,
        );
    }
    ...
    const transformStream = new Transform({
        ...
        transform(data, enc, cb) {
            ...
            objects.push(data);
            console.log(
                `Número actual de objetos procesados: ${objects.length}`,
            );
            cb();
        },
    });
    ...
    jsonStream.on("error", (error) =>
        console.error("Error in jsonStream:", error),
    );
    parser.on("error", (error) => console.error("Error in parser:", error));
    transformStream.on("error", (error) =>
        console.error("Error in transformStream:", error),
    );
    ...
    return new Promise((resolve, reject) => {
        jsonStream
            ...
            .on("finish", () => {
                console.log(
                    "Todos los datos han sido procesados exitosamente.",
                );
            ...
            .on("error", (error) => {
                ...
                console.error(
                    `Se procesaron un total de ${objects.length} objetos antes de fallar.`,
                );
                reject(error);
            });
    });
...
const getWindMapData = async (req, res) => {
    ...
    try {
        ...
        if (response.status !== 200) {
            throw new Error(
                "No se pudieron obtener los datos del mapa eólico ibérico.",
            );
        }
        ...
        const chartData: { angle: string; total: number }[] =
            windDirections.map((direction, dirIndex) => {
                const directionData = { angle: direction, total: 0 };

                windHeights.forEach((height, heightIndex) => {
                    const value =
                        parseFloat(
                            windMapData.wind_rose.data.yhist2[heightIndex][
                                dirIndex
                            ],
                        ) || 0;
...
const getExclusionMapData = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (response.status !== 200) {
            throw new Error(
                "No se pudieron obtener los datos del mapa de exclusión eólica.",
            );
        }
