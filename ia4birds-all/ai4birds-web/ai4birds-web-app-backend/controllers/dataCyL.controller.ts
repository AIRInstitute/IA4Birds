...
const getXenoCantoRecordings = async (req, res) => {
    ...
    try {
        ...
        if (response.status !== 200) {
            throw new Error(
                "No se pudieron obtener las grabaciones de Xenocanto.",
            );
        }
...
const getEBirdData = async (req, res) => {
    ...
    try {
        ...
        if (response.status !== 200) {
            throw new Error(
                "No se pudieron obtener los datos de avistamientos de aves de eBird.",
            );
        }
...
const getDataBird = async (req, res) => {
    ...
    try {
        ...
        if (cachedData) {
            ...
            const dataBirdResponse = await axios.get(
                `${globalConfig.pythonURL}/dataBird`,
            );
...
const getSensitivityData = async (req, res) => {
    ...
    try {
        ...
        const sesitivityResponse = await axios.get(
            `${globalConfig.pythonURL}/sensitivity`,
        );

        // Verificar si la solicitud fue exitosa
        if (sesitivityResponse.status !== 200) {
            throw new Error(
                "No se pudieron obtener los datos de sensibilidad.",
            );
        }
