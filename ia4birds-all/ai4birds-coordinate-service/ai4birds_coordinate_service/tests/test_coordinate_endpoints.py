...
import requests
from config_test import BASE_URL, TOKEN, EXPECTED_STATUS_200_500, EXPECTED_STATUS_200_500_404
...
def test_ebird(mock_requests):
    ...
    assert response.status_code in EXPECTED_STATUS_200_500
...
def test_databird(mock_requests):
    ...
    assert response.status_code in EXPECTED_STATUS_200_500_404

@pytest.mark.parametrize("body", [
    {"lat": 41.85563222906876, "lon": -5.5495918821608665, "z": 50}
])
def test_windmap(mock_requests, body):
...
def test_exclusionmap(mock_requests, body):
...
def test_device_status(mock_requests, body):
